...
description: Locale-aware routing, detection, and translation with alab/i18n.
sidebar:
  order: 2
---

Alab's i18n module adds locale prefixes to your routes (`/en/`, `/fil/`, etc.), detects the user's preferred language from the `Accept-Language` header, and provides React context for the active locale.
...
With this config, Alab rewrites your route manifest so every route is available under a locale prefix:
...
```tsx
import { useLocale } from "alab/i18n";
...
```tsx
import { LocaleLink } from "alab/i18n";
...
import { detectLocale } from "alab/i18n";
...
Alab does not ship a built-in translation runtime. Use any i18n library alongside the locale context:
