...
3. It extracts the `#alab-root` inner content, `<title>`, and `<meta>` tags from the response
4. `history.pushState` updates the browser URL
5. The `#alab-root` content is swapped in with a short CSS transition
...
The swap uses a CSS `page-enter` animation class added to `#alab-root` during the transition. You can customise it in your `globals.css`:
