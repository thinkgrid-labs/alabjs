// Canonical post data — shared by the posts list and post detail server functions.
// In a real app, replace this with a DB/ORM call.
...
Rust changes this. By compiling TypeScript with oxc — a Rust-native parser and transformer — AlabJS achieves cold starts under 100ms on large codebases. There's no JIT warmup, no garbage collector pauses.
...
The combination is powerful: a compiler that never becomes the bottleneck, paired with a rendering model that meets the user where they are.`,
  ...
  },
  {
    ...
    body: `AlabJS is Filipino for blaze. That's not a marketing angle — it's a design philosophy.
...
The goal is a framework where the path of least resistance is also the path of best practice. No plugin archaeology. No config file archaeology. Just write React and let AlabJS handle the rest.`,
    author: "AlabJS Team",
  ...
  },
  {
    ...
    excerpt: "Explicit is better than implicit. AlabJS's server boundary system gives you the guarantees of React Server Components without the hidden rules.",
...
AlabJS takes a different approach. Server functions are defined explicitly with defineServerFn. The Rust compiler statically verifies that no server module is imported on the client. The boundary is opt-in and type-checked.
