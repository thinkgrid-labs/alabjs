...
declare global {
  interface Window {
    __alab_navigate?: (href: string) => Promise<void>;
  }
}

/**
 * Client-side navigation link for Alab.
 *
...
export function Link({ href, children, prefetch = true, onClick, ...rest }: LinkProps) {
  ...
  const handleClick = async (e: MouseEvent<HTMLAnchorElement>) => {
    ...
    if (typeof window.__alab_navigate === "function") {
      await window.__alab_navigate(href);
    } else {
