...
describe("htmlShellBefore", () => {
  ...
  it("produces valid HTML doctype and structure", () => {
    ...
    expect(html).toContain('<div id="alab-root">');
  });

  it("includes alab-route meta tag", () => {
    const html = htmlShellBefore(baseOpts);
    expect(html).toContain('<meta name="alab-route" content="app/page.tsx" />');
  });

  it("includes alab-ssr meta tag set to false", () => {
    const html = htmlShellBefore(baseOpts);
    expect(html).toContain('<meta name="alab-ssr" content="false" />');
  });

  it("includes alab-ssr meta tag set to true", () => {
    const html = htmlShellBefore({ ...baseOpts, ssr: true });
    expect(html).toContain('<meta name="alab-ssr" content="true" />');
  });
  ...
  it("embeds params JSON", () => {
    ...
    expect(html).toContain('<meta name="alab-params" content="{&quot;id&quot;:&quot;42&quot;}" />');
  });
  ...
  it("includes Twitter Card tags", () => {
    ...
    expect(html).toContain('<meta name="twitter:creator" content="@alab" />');
  });
  ...
  it("includes layouts meta tag when provided", () => {
    ...
    expect(html).toContain('name="alab-layouts"');
  });
  ...
  it("includes loading meta tag when provided", () => {
    ...
    expect(html).toContain('<meta name="alab-loading" content="app/loading.tsx" />');
  });
...
describe("htmlShellAfter", () => {
  it("closes the alab-root div and body/html", () => {
  ...
  it("includes the client script tag", () => {
    const html = htmlShellAfter({});
    expect(html).toContain('<script type="module" src="/@alab/client">');
  });
