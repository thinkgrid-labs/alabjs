/**
 * @alabjs/sync — Local-first sync engine adapters for Alab.
 *
 * Provides a common React context + hooks interface over three sync backends:
 *
 * | Backend    | Import                  | Best for                              |
 * |------------|-------------------------|---------------------------------------|
 * | PGlite     | `@alabjs/sync/pglite`     | Offline-first, no server required     |
 * | ElectricSQL| `@alabjs/sync/electric`   | Postgres → client sync (shapes)       |
 * | PowerSync  | `@alabjs/sync/powersync`  | Postgres/Supabase → SQLite in browser |
 *
...
// Re-export adapter entry points so callers can import from "@alabjs/sync" directly.
