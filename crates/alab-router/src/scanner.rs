...
pub fn scan_routes(app_dir: &str) -> RouteManifest {
    ...
    // This can happen on case-insensitive file systems (e.g. macOS, Windows)
    // where `Page.tsx` and `page.tsx` resolve to the same URL.
    let mut seen: std::collections::HashMap<(&str, u8), &str> = std::collections::HashMap::new();
    for route in &routes {
        let kind_key: u8 = match route.kind {
            RouteKind::Page => 1,
            RouteKind::Api => 2,
            _ => continue, // layouts/server/error/loading don't need uniqueness checks
        };
        let key = (route.path.as_str(), kind_key);
        if let Some(existing_file) = seen.get(&key) {
            panic!(
                "[alabjs] Route collision: '{}' is generated by both '{}' and '{}'. \
                 Each URL path must map to exactly one page or API route.",
                route.path, existing_file, route.file
            );
        }
        seen.insert(key, &route.file);
    }
